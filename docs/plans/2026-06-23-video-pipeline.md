...
> **方案脚本格式(所有时长统一,2025 最新)**:不向模型要 freeform `storyboard`(它会甩 `【0-3秒】` 时间轴、且 5/10/15 格式漂移),而是要【每拍一句连贯动作】(`shot1..shotN`,每拍约5s、"别写 0-x 秒内部时间戳"),**后端按固定格式合成** `storyboard`(`【分镜①·0-5s】… ②·5-10s… ③·10-15s…`)。N=拍数:**15s→3、10s→2、5s→1**。→ 默认必然精简、格式一致(治"10s 总带详细时间轴、15s 却不带")。详细时间轴由卡片上的**「详细扩展」**按需展开(`expand_proposal`,扣 1 点)。5/10s 单镜生成只用合成后的 `storyboard`(不暴露 shot 字段);15s 才用 `shot1/2/3` 拆 3 段并行 + per-shot 母帧。
