...
def test_export_default_all_formats(client, auth_headers, png):
...
# ---- 自定义尺寸 / DPI / 格式子集 -----------------------------------------
def test_export_custom_size_dpi_subset(client, auth_headers, png):
...
# ---- 出血:全幅画布 = 裁切 + 2×出血 -------------------------------------
def test_export_bleed_expands_canvas(client, auth_headers, png):
...
# ---- 默认无出血:全幅 == 裁切(向后兼容) --------------------------------
def test_export_no_bleed_canvas_equals_trim(client, auth_headers, png):
...
# ---- 排版模式 / 锚点 -------------------------------------------------------
def test_export_scale_anchor_modes(client, auth_headers, png):
    for scale in ("contain", "cover", "actual"):
        for anchor in ("center", "top", "bottom"):
            ...
            assert r.status_code == 200, f"{scale}/{anchor}: {r.text}"
            assert r.json()["meta"]["scale"] == scale
            assert r.json()["meta"]["anchor"] == anchor


# ---- CMYK:jpg 输出确为 CMYK 模式;png 仍 RGB ----------------------------
def test_export_cmyk_jpg(client, auth_headers, png):
...
# ---- 打样核对图 -----------------------------------------------------------
def test_export_proof_image(client, auth_headers, png):
    r = _post(client, auth_headers, png, formats="png", width_cm="10", height_cm="10",
...
# ---- 安全边过大(塌缩)→ 400 + 退点 -------------------------------------
def test_export_safe_too_large_refund_400(client, auth_headers, png):
    before = _balance(client, auth_headers)
    # 2cm 成品 + 安全边 15mm → 2×15mm 已 >= 20mm 成品 → 塌缩
