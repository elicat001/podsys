...
def scene_frame_prompt(language: str = "葡萄牙语", scene: str = "", action: str = "") -> str:
    """场景母帧的 gpt-image 指令:生成视频【真正的第 0 帧】(脚本开头那一刻),把商品放进真人正在真实生活里使用它的场景。
    ⚠ Scene Init(最优先):母帧=视频第0帧、不是独立成品图。给了 action(脚本)就让首帧落在它的【起始瞬间】、与脚本开头衔接。
    ⚠ 通用·自适应,绝不写死品类/动作/人物属性:人物年龄性别、场景、互动方式都由模型【看商品自己判断】——
    这样换任意 SKU 都成立(解压球→有人要按压旋转它;杯子→有人要端起;T恤→有人穿着),不是硬编码"小孩转球"。
    scene:智能向导可传入【具体场景+人物状态】(产品驱动);留空则由模型看图自适应。action:该镜脚本,用于对齐起始状态。"""
    ...
    action_line = (
        f"【这一帧是这段视频的第 0 帧】视频开头的动作是:『{action[:400]}』——只呈现它【最开始那一刻】的状态"
        "(动作即将开始、尚未展开;人物在起始位置、商品在动作开始前该在的位置与状态),绝不画到动作中段或结果。"
    ) if action else ""
    return (
        "把图中的商品自然地放进一个【真人正在真实生活里使用或把玩它】的场景,作为短视频的【第一帧(第 0 帧)】。"
        + scene_line + action_line +
        f"让一个{rt}真人(年龄、性别贴合这件商品的真实目标用户,由你看图判断、自然合理)在贴合该商品的生活环境里"
        "(居家桌前/客厅/户外等),正要上手使用或把玩这个商品,"
        "【正处在脚本开头那一刻——动作即将开始的瞬间】(不是静止摆拍 pose、也不是动作做完的结果),神态自然放松、像没意识到在拍。"
        "完整保留商品的图案、文字、颜色与形状(产品本体不可改动、丢失或变形),商品以真实立体形态出现、不平铺悬空。"
        "【仅限『难动作』前移,按需不强制】若这件商品要用起来需要一个很容易画坏/穿模的机械状态变化"
        "(拧开瓶盖、拉开拉链、穿脱、拆封、倒出液体),就让首帧直接呈现该动作已完成、可直接使用的状态(瓶盖已开/已穿上/已打开);"
        "但像按压、旋转、捏压回弹这类把玩互动是这类模型的强项,【不要】前移,正常呈现『正进行中』即可。商品本体与原图完全一致、只是状态不同。"
        "【首帧身份锚点】画面里这件商品清晰、完整、只出现一件(同一个个体,不重复/不分身),有真实支撑、不悬空——给后续视频稳定的对象与物理锚点。"
