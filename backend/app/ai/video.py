...
CATEGORY_ACTIONS: dict[str, str] = {
    ...
    # 水杯/保温杯常带盖,而开盖是 AI 视频最易翻车的动作 → 动作刻意只做旋转/展示,不写"拧开杯盖"(对齐物理优化)。
    "水杯": "拿起水杯缓慢旋转一周展示杯身印花,手握杯子展示握感与容量,镜头推近展示瓶身材质与细节,自然放回桌面。",
...
NEGATIVE_BLOCK = (
    ...
    "商品被改样/拉伸/变形、低质量画面;"
    "以及一切违背物理常识的现象:杯盖/瓶盖/拉链/纽扣/包装等部件无人触碰却自行开合、移动或凭空脱离,"
...
_QUALITY_SUFFIX = (
    ...
    "全程一切动作与物体交互都遵循真实物理与重力规律:运动连续自然、不跳变,手与物体保持稳定接触;"
    "商品的形态、部件与开合状态始终保持不变,除非画面中有手在明确、连贯地操作它(且操作过程逐帧可见、不跳步);"
