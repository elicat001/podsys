...
CATEGORIES: list[str] = ["通用", "T恤", "卫衣", "马克杯", "水杯", "手机壳", "帆布袋", "海报", "抱枕"]
...
# 物理连贯段是「通用」的:无论什么商品/动作都追加,治的是 AI 视频通病(部件自行开合、物体瞬移漂浮),
# 列举的杯盖/拉链等只是示例帮模型理解,不是按品类写死的规则。
NEGATIVE_BLOCK = (
    "请务必避免:人物僵硬或静止不动、机械重复动作、呆板摆拍感、商业广告片或 CG 动画质感、"
    "畸形或多余的手指手部、面部模糊扭曲、过度美颜磨皮、"
    "商品的图案/文字被改样/拉伸/扭曲、低质量画面;"
    "【尤其避免「像一张会动的 AI 照片」+「模特摆拍」】:镜头固定死机位、全程一动不动、只有主体在原地微动、"
    "人物正中端站着、全程正对镜头傻笑、举着商品对镜头展示、走秀展示感、像在完成拍摄任务而不是过生活;"
    "【尤其避免一切错误的材质物理】:衣物/布料像纸板或砖块般僵硬、平铺悬空、整件刚性平移或翻面、毫无垂坠褶皱与飘动;"
    "蓬松/弹性物体被压后不回弹、永久塌陷、或完全不形变(失去弹性);刚性硬物像果冻般软塌、扭曲或晃动;"
    "液体凝固不流动、悬空成块或穿出容器;反光表面高光僵死不随角度变化;"
    "以及违背物理常识的现象:杯盖/瓶盖/拉链/纽扣/包装等部件无人触碰却自行开合、移动或凭空脱离,"
    "物体凭空出现或消失、瞬移、穿模、漂浮,手与物体接触中断、动作跳帧不连贯,任何不符合重力与真实物理规律的运动。"
)

# 材质物理(治"平面像砖块""弹性失真"等通病):不按品类写死,而是让模型按【画面中商品的实际材质】
# 表现对应物理行为 —— 软布垂坠、蓬松回弹、硬物坚固、液体晃荡、反光移动、薄纸卷曲 + 重力/惯性/接触通则。
_PHYSICS_BLOCK = (
    "全程严格遵循真实世界的材质物理,按画面中商品的【实际材质】表现对应的物理行为:"
    "柔软布料(衣物/帆布袋/窗帘/枕套等)随身体动作与微风自然垂坠、起伏、产生真实褶皱与轻柔飘动,有重量感;"
    "蓬松与弹性材质(抱枕/海绵/针织等)被按压或倚靠时自然凹陷形变、松手后【弹性回弹复原】,既非纹丝不动也非永久塌陷;"
    "刚性硬物(陶瓷/玻璃杯/手机壳/塑料等)坚固不形变、有真实重量与惯性,绝不像果冻般软塌晃动;"
    "液体(杯中饮品等)随容器移动而晃荡、有液面起伏与涟漪,倾倒时连续顺滑流动、不凝固不悬空;"
    "光滑反光表面(玻璃/釉面/屏幕/金属)的高光与反射随角度自然移动;薄片纸张(海报/标签)边缘可随风轻微卷曲抖动而非铁板一块;"
    "一切运动都遵循重力、惯性与接触关系:物体有重量、起停有加速与缓冲、手与物体保持稳定接触。"
)
...
_QUALITY_SUFFIX = (
    ...
    "商品的图案、文字、颜色与设计细节始终保持一致,不凭空增减、改样或被拉伸扭曲;"
    "部件(杯盖/拉链/纽扣/包装等)不在无人触碰时自行开合或脱离。"
    "光线自然,商业级高画质、真实可信。"
)

# 真人感(Human Presence)—— 核心杠杆:TikTok 的"活跃"主要来自【人物行为】而非运镜。
# 人物要在"过自己的生活、做一件具体的事",商品是顺带穿用的道具,不是被展示的对象;并破除 AI 通病
# "全程看镜头傻笑摆拍"。通用行为词汇库 = 给模型的 grounding(按情境自选),【不按场景写死】。
_BEHAVIOR_BLOCK = (
    "【人物行为(最重要)】画面里的人要像在过自己的真实生活、专注做一件具体的事,而不是当模特对镜头摆拍、举着商品展示。"
    "用具体、低风险的生活化动作推进画面:整理衣领、拉一下衣角、转身、甩头发、随意摆个 pose、"
    "端起杯子喝一口、低头看手机、推门、走动、把包甩上肩、回头看一眼……按当前情境自然选 1~2 个。"
    "【破除「摆拍感」】别为了展示商品而僵硬地正对镜头摆拍、把商品举到镜头前怼着展示;人物注意力在自己做的事上,视线自然——"
    "生活类内容里人常常并不一直看镜头、像没意识到正在被拍(真实 UGC 抓拍感);口播/种草类可以自然地对镜头说话,但同样别僵硬摆拍。"
    "商品作为这段生活里自然穿着/使用的道具清晰出现,不必刻意举到镜头前。"
)

# 镜头语言(次要,GPT 优先级 #4):保留基础手持运镜即可、不再加码 —— 真人感主要靠【行为】不靠运镜。
_CAMERA_BLOCK = (
    "【镜头】像真人手持手机随手拍:有自然手持感(轻微晃动、随手调角度),镜头与人至少一方在动,"
    "别固定死机位、别像会动的静态照片;运动自然、不眩晕。"
)
...
def compose_prompt(motion: str = "", language: str = "葡萄牙语") -> str:
    """专业化拼装:镜头脚本(用户填/改)+ 地区 UGC 风格(按语言)+ 语言
    + 全局一致性/自然开场/防拉伸 + 负向词 → 最终 prompt。语言/画幅是独立选项,不被描述冲掉。
    (动作完全交给用户的镜头脚本/视频描述,不再按类目追加固定动作,避免与描述打架。)"""
    parts: list[str] = []
    ...
    parts.append(_BEHAVIOR_BLOCK)      # 人物行为(最重要):真人感,破除"模特对镜头摆拍"
    parts.append(_QUALITY_SUFFIX)
    parts.append(_CAMERA_BLOCK)        # 镜头(次要):基础手持运镜
    parts.append(_PHYSICS_BLOCK)       # 材质物理(软布/弹性/刚性/液体/反光/纸张 + 重力惯性)
