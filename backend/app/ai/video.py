...
NEGATIVE_BLOCK = (
    ...
    "【尤其避免「像一张会动的 AI 照片」】:镜头固定死机位、全程一动不动、只有主体在原地微动、"
    "人物正中端站着对镜头展示、整段像静态照片加轻微动效——这正是要消除的呆板感;"
...
# 镜头语言(治"AI 照片活了":主体静 + 镜头静 = 照片微动)。镜头运动是 CogVideoX 的【安全杠杆】
# (比主体大动作风险低),也是 TikTok"有味道"的关键。通用原则(不按场景写死),具体运镜由脚本/向导按
# 情境生成;研究最佳实践:镜头指令与主体动作【分开写】,模型最听话 → 故单列一段。
_CAMERA_BLOCK = (
    "【镜头语言】整段要像真人手持手机随手拍,不是固定三脚架死机位、也不是只有微动的静态照片:"
    "镜头有自然的手持感(轻微晃动、呼吸般起伏、随手微调角度);这一拍内要有【主动的镜头运动】——"
    "从 推近/拉远、轻微摇移跟随主体、变换机位与视角(怼近自拍角度、侧身跟拍、低机位、俯拍桌面、回头) 中,"
    "按当前情境自然选用 1~2 个组合;镜头与主体至少有一方在持续运动,二者不要同时僵住。"
    "节奏上:开头约 1 秒先用一个吸睛的镜头动作抓住注意力,中段推进,结尾自然收;"
    "运动幅度自然、不眩晕、不甩飞,保持 UGC 真实抓拍质感。"
)
...
def compose_prompt(motion: str = "", language: str = "葡萄牙语") -> str:
    ...
    parts.append(_CAMERA_BLOCK)        # 镜头语言(手持 UGC + 主动运镜 + 节奏弧;治"AI 照片活了")
