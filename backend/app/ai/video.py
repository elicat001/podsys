...
_DIRECTION_BLOCK = (
    ...
    "【动作要连成一条因果链】人物从一个动作自然过渡到下一个(如 看手机→起身→拿钥匙→推门),"
    "镜头切换时动作自然延续下去,让人觉得「他正在经历一件事」,而不是停在一个 pose、或拼接几个互不相关的片段;"
...
def compose_prompt(motion: str = "", language: str = "葡萄牙语") -> str:
    ...
    parts.append(_DIRECTION_BLOCK)     # 导演层(正向为主):身份=记录真实生活 + 任务动作 + 手持镜头
