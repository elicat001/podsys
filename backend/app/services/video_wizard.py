...
def describe_product(image: Image.Image, selling_points: str = "", language: str = "葡萄牙语") -> dict:
    ...
    prompt = (
        "你是跨境电商(按需印制 POD)选品与文案专家。仔细观察这张商品图,提炼出用于拍带货视频的【商品简报】。\n"
        "【识别原则(通用 · 关键):先分清『直接可见』与『经验推断』,再决定怎么写】\n"
        "· 直接可见——图中确实看得到的(印在商品上的图案/图像/文字、颜色、造型外形、可见部件如手柄/盖子/吸管、表面纹理质感等)"
        "→ 作为【确定事实】准确描述。\n"
        "· 经验推断——单凭外观无法确认、只能靠品类常识去猜的(具体材质成分、内部结构、是否保温/防摔、功能参数等)"
        "→ 【绝不写成确定事实】;要么不写,要么明确标注是『推测/可能』。别把『猜的』当成『看到的』。\n"
        "⚠ POD 印花商品:商品上印的【图案/设计】通常是它最独特、最该卖的点,而且它恰好属于『直接可见』——"
        "若有印花,务必先看清并准确描述(主题、文字、配色、风格),把它作为【首要卖点】;若确实没有印花,再按普通商品描述。\n"
...
def generate_proposals(name: str, audience: str, selling_points: str, *, seconds: int = 10,
    ...
    if two_shot:
        time_rule = (
            ...
            "【关键帧渐进(按需,不强制)】若这件商品需要一个很容易画坏/穿模的机械动作(开盖/拆封/穿脱)才能开始用,"
            "可善用三分镜的镜头切换把它藏在镜头之间:分镜①给完整/未开封的商品展示(scene① 描述完整状态),"
            "分镜②③直接是【已就绪/使用中】的状态(scene②③ 描述已开盖/已穿上的可用状态)——让难动作发生在『切一下』之间,"
            "而不是在某一镜里现场做。若商品本就能直接用(无难动作),就按最自然的连续动作链来,不必这样安排。\n"
