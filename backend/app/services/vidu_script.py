...
def describe_multishot(image: Image.Image, seconds: int = 10, language: str = "葡萄牙语",
    ...
    prompt = (
        ...
        "④ 动作贴合 AI 视频能力边界:【按需】若这件商品需要一个很容易画坏/穿模的机械状态变化(开盖/拆封/穿脱/倾倒)才能开始用,"
        "就默认它在首帧已完成、从可直接使用的状态写起(别写现场开盖/拆封/穿脱);但按压、旋转、捏压回弹这类把玩互动是 Vidu 的强项,正常大胆写、不必前移;"
        "除此之外的自然动作都【大胆做、有真实运动幅度】,手与物体接触、符合重力,绝不出现部件自行开合或物体凭空出现/消失;商品的图案、文字、颜色必须保持一致、不被改样;\n"
