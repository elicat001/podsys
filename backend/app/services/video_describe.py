...
def smart_describe(image: Image.Image, video_type: str = "开箱分享", seconds: int = 10,
    ...
    prompt = (
        ...
        "④ 动作要贴合 AI 视频的能力边界(这是所有商品的通用约束,不只针对某一类):【按需】若这件商品需要一个很容易画坏/穿模的"
        "机械状态变化(开盖/拧盖、拆封开箱、组装拆卸、穿脱、折叠、倾倒液体)才能开始用,就默认它在首帧已完成、脚本【从可直接使用的状态写起】"
        "(别写『现场拧开瓶盖/拆开包装/把衣服穿上』这类机械过程);除此之外按商品真实用法大胆设计【有真实运动幅度、有进展】的自然动作、"
        "让画面有动感而非几乎不动;让手与物体接触、动作连续且符合重力,绝不出现部件无人触碰却自行开合、或物体凭空出现/消失/移动;\n"
