...
# corner_threshold 越大越平滑(小于该角度的拐点都拟合成圆顺曲线);length_threshold 越大线段越长越平滑。
# 扁平图(logo)要"圆顺描边"→ 大 corner/length;照片档要保细节 → 小 length 别把发丝拉直。
_PRESETS = {
    # 扁平图形(logo/卡通/印花稿):低色深收拢 AA 过渡色 + 高 corner/length → 描边干净、圆顺、无碎边
    "logo": dict(filter_speckle=10, color_precision=6, layer_difference=16, corner_threshold=90, length_threshold=8.0, path_precision=2),
    # 默认高保真(照片/插画):近原生 + 低去碎块 + 高色深 + 小 length → 保留细节/毛发、放大无锯齿
    "illustration": dict(filter_speckle=4, color_precision=8, layer_difference=8, corner_threshold=40, length_threshold=4.0, path_precision=3),
    # 极致细节(毛发/照片):几乎不去碎块、最高色深、最锐角、最小 length;代价是路径多、SVG 大、较慢。
    "fine": dict(filter_speckle=1, color_precision=8, layer_difference=6, corner_threshold=30, length_threshold=4.0, path_precision=3),
}
# 扁平图(logo)描摹前的均值漂移(pyrMeanShiftFiltering)参数:sp=空间半径、sr=颜色半径。
# 把描边旁的 AA 过渡带抹进主色 → 描边均匀无残影。越大合并越狠(过大会吃掉细印花),16/40 实测稳。
_MS_SP, _MS_SR = 16, 40
...
def _vtracer_svg(rgb: Image.Image, orig_w: int, orig_h: int, preset: str = "auto") -> tuple[str, int]:
    ...
    # ⚠️ 照片/插画档不做去噪(双边/中值都会抹掉发丝/细线条)。细小噪点交给 vtracer 的 filter_speckle。
    arr = np.asarray(work)
    # 🔑 扁平图(logo)专属:均值漂移把描边旁的 AA 过渡带『抹进』两侧主色 → 描边均匀、无灰色残影、
    #   边缘平滑(治"一粗一细 + 线条阴影")。它产出锐利边界的平整色块,正合扁平图;照片档绝不做(会糊细节)。
    if kind == "logo":
        try:
            import cv2
            arr = cv2.pyrMeanShiftFiltering(arr, _MS_SP, _MS_SR)
        except Exception:  # noqa: BLE001  均值漂移失败不致命 → 退回未滤波(仍出图,边缘略带过渡)
            pass
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        vtracer.convert_image_to_svg_py(
            ...
            splice_threshold=45, **params,   # length_threshold/corner_threshold 已按档位放进 params
        )
