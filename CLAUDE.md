...
- `app/tasks.py`:worker 侧顶层任务。**闭包不能跨进程**,故任务只收 `job_id`,自己按 id 从盘上读输入
  ...
  - `run_print_extract`(印花提取专用)+ `run_tool`(其余 13 个工具共用,按 `job.kind` 在 `TOOL_WORKS`
    注册表里分派 `_work_*`)。
...
# 2. 起 worker(Windows 必须 --pool=solo,prefork 在 Win 不稳):
cd D:\podsys\backend; .\.venv\Scripts\celery.exe -A app.celery_app worker -l info --pool=solo
